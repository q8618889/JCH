...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _chartXArray = [NSMutableArray array];
        _gradesArray = [NSMutableArray array];
...
- (void)initData
{
    ...
    _minColumnColor = BBCOLOR;
    _maxColumnColor = BBCOLOR;
    _normalColumnColor = BBCOLOR;
...
- (void)reloadData
{
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        if (nil == eColumn)
        {
            eColumn = [[EColumn alloc] initWithFrame:CGRectMake(widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5)+20, 30, 20, self.frame.size.height-30)];
            CGFloat numberX =widthOfTheColumnShouldBe * 0.5 + (i * widthOfTheColumnShouldBe * 1.5)+20;
            [_chartXArray  addObject:[NSString stringWithFormat:@"%f",numberX]];
...
//            }
            ...
            _grades = eColumn.grade;
            CGFloat gradess = eColumn.grades;
            [_gradesArray addObject:[NSString stringWithFormat:@"%f",gradess]];
